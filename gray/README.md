# gray
### Introduction
We use gray codes to design mappings x-y coordinates to note space.
The goal is to make a music interface where a MIDI controller or mousepad controls musical notes in an interesting way.
...
### Gray codes
A [Gray code](https://en.wikipedia.org/wiki/Gray_code) encodes the integers as binary vectors such that adjacent word pairs in the code shares all but one bit.
...
We consider the binary code words as representing whether each note in a set of valid notes is on or off.
...
### 2-D gray codes
We extend Gray codes into 2-D in order to come up with interesting position-to-notes mappings.


### Max/MSP patches
* `2d.maxpat`
 * maps x-y space (quantized to an 8-by-8 square) onto 7-bit binary vectors according to a 2-D Gray code.
 * The binary code words control whether each note in a configurable set is on or off.
